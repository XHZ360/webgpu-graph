# schema-graph-inspector Specification

## Purpose

Define a read-only schema graph inspector that uses the editor inspection contract as the source for schema summaries, graph nodes and edges, and selected-node details while remaining separate from WebGPU runtime execution.
